...
class MemoryQueue {
	...
	async getStats(): Promise<{ size: number; oldestEvent?: number }> {
		...
		const oldestEvent = size > 0 ? Math.min(...this.queue.map((e) => e.timestamp)) : undefined;
...
export class EventQueue implements EventQueueInterface {
	...
	private async removeExpiredRetries(): Promise<void> {
		if (this.useIndexedDB && this.db) {
			...
			const expiredIds = await this.memoryQueue.getEventsExceedingRetries(this.config.maxRetries);
			if (expiredIds.length > 0) {
