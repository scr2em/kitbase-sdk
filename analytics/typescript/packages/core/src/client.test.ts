...
import { KitbaseAnalytics } from "./client.js";
import { ValidationError, AuthenticationError, ApiError, TimeoutError } from "./errors.js";

describe("KitbaseAnalytics", () => {
	...
	describe("track", () => {
		it("should throw ValidationError when event is missing", async () => {
			const client = new KitbaseAnalytics({ sdkKey: "test-token" });
			await expect(client.track({ channel: "test", event: "" })).rejects.toThrow(ValidationError);
		});
		...
		it("should successfully track an event", async () => {
			...
			expect(mockFetch).toHaveBeenCalledWith(
				"https://ingest.kitbase.dev/sdk/v1/logs",
		...
		it("should throw AuthenticationError on 401", async () => {
			...
			await expect(client.track({ channel: "test", event: "Test Event" })).rejects.toThrow(
				AuthenticationError,
			);
		});
