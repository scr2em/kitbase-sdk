...
/**
 * Hook to check if the user has opted out of tracking.
 *
 * @returns Whether tracking is opted out
 *
 * @example
 * ```tsx
 ...
 *   return <div>You have opted out of tracking</div>;
 * }
 * ```
 */
export function useOptedOut(): boolean {
  const kitbase = useKitbaseAnalytics();
  return kitbase.isOptedOut();
}

/**
 * Hook to get opt-in and opt-out functions for consent management.
 *
 * @returns Object with optIn and optOut functions
 *
 * @example
 * ```tsx
...
export function useConsent() {
  ...
  return useMemo(
    () => ({
      optIn: () => kitbase.optIn(),
      optOut: () => kitbase.optOut(),
      hasConsent: () => kitbase.hasConsent(),
    }),
    [kitbase]
  );
}
