...
export async function resolveConfig(env: McpEnv): Promise<McpConfig> {
	...
	if (!response.ok) {
		...
		throw new Error(
			`Failed to resolve API key info (${response.status}): ${text || response.statusText}`,
		);
	}
	...
	if (!keyInfo.orgSlug) {
		throw new Error(
			"API key info did not return an organization slug. Please ensure your API key is valid.",
		);
	}
