	...
	server.tool(
		...
		"Get web analytics summary KPIs (visitors, pageviews, bounce rate, etc.) with percentage changes compared to previous period",
		{
			...
			filters: z
				.array(z.string())
				.optional()
				.describe('Filters in format "dimension:operator:values" (e.g., "country:is:US")'),
		},
		async (params) => {
			try {
	...
	server.tool(
		...
		"Get web analytics metrics over time (timeseries data for charting trends)",
		{
			metric: z
				.string()
				.describe(
					'Metric to chart (e.g., "visitors", "pageviews", "bounce_rate", "visit_duration", "views_per_visit")',
				),
			preset: z
				.enum([
					"last_30_minutes",
			...
			filters: z
				.array(z.string())
				.optional()
				.describe('Filters in format "dimension:operator:values"'),
		},
		async (params) => {
			try {
