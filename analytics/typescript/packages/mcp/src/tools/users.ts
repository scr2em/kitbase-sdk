...
export function registerUsersTools(server: McpServer, client: KitbaseApiClient): void {
	...
	server.tool(
		...
		"List or search users in the project",
		{
			...
			preset: z
				...
				.optional()
				.describe("Date preset for filtering by activity"),
	...
	server.tool(
		...
		"Get comprehensive analytics summary for a specific user (total events, first/last seen, top events, etc.)",
		{
			...
			type: z
				...
				.describe("Whether the user is identified or anonymous"),
		},
		async (params) => {
			try {
				const result = await client.request(
					`/projects/{projectId}/analytics/users/${encodeURIComponent(params.userId)}/summary`,
					{
						params: {
							type: params.type,
						},
	...
	server.tool(
		...
		async (params) => {
			try {
				const result = await client.request(
					`/projects/{projectId}/analytics/users/${encodeURIComponent(params.userId)}/activity`,
					{
						params: {
							type: params.type,
	...
	server.tool(
		...
		async (params) => {
			try {
				const result = await client.request(
					`/projects/{projectId}/analytics/users/${encodeURIComponent(params.userId)}/events`,
					{
						params: {
							type: params.type,
							page: params.page,
