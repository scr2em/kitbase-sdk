		...
		<div class="grid">
			<div class="card">
				...
				<p style="color: #8b949e; margin-bottom: 1rem">
					Navigate between pages to test automatic page view tracking. The core SDK intercepts
					history.pushState/popstate and tracks page views automatically.
				</p>
			...
			<div class="card">
				...
				<div style="font-size: 0.85rem; line-height: 1.8">
					<div>
						Debug mode: <strong>{{ kitbase.isDebugMode() }}</strong>
					</div>
					<div>
						User ID: <strong>{{ kitbase.getUserId() ?? "(none)" }}</strong>
					</div>
					<div>
						Active timers: <strong>{{ kitbase.getTimedEvents().length }}</strong>
					</div>
					<div>
						Plugins: <strong>{{ kitbase.getPlugins().join(", ") || "(none)" }}</strong>
					</div>
				</div>
			</div>
		</div>
