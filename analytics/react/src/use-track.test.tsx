...
describe("useTrack", () => {
	...
	it("should initialize with default state", () => {
		const { result } = renderHook(() => useTrack(), { wrapper: createWrapper(mockClient) });
...
describe("useTrackChannel", () => {
	...
	it("should initialize with default state", () => {
		const { result } = renderHook(() => useTrackChannel("payments"), {
	...
	it("should use different channels for different hooks", async () => {
		const { result: result1 } = renderHook(() => useTrackChannel("payments"), {
			wrapper: createWrapper(mockClient),
		});

		const { result: result2 } = renderHook(() => useTrackChannel("signups"), {
			wrapper: createWrapper(mockClient),
		});
	...
	it("should update trackChannel function when channel changes", async () => {
		const { result, rerender } = renderHook(({ channel }) => useTrackChannel(channel), {
			wrapper: createWrapper(mockClient),
