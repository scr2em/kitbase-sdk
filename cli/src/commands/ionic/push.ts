...
import {buildAndZip, getNativeVersion, validateZipFile, cleanupTemp, isIonicProject} from '../../ionic/build.js';
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (flags.commit && flags.branch) {
        gitInfo = {commitHash: flags.commit, branchName: flags.branch, commitMessage: flags.message};
      } else {
        if (!isGitRepository()) {
          spinner.fail('Not a git repository');
          throw new GitError('Not a git repository. Run from a git repo or provide --commit and --branch.');
        }
      ...
      spinner.succeed(`Git: ${chalk.dim(gitInfo.branchName)} @ ${chalk.dim(gitInfo.commitHash.substring(0, 7))}`);
      ...
      if (flags.file) {
        ...
        spinner.succeed(`Using zip: ${chalk.dim(zipFilePath)} ${chalk.cyan(`(${formatSize(zipSize)})`)}`);
      } else {
        spinner.stop();
        const result = await buildAndZip({skipBuild: flags['skip-build'], outputDir: flags['output-dir']});
        zipFilePath = result.zipPath;
        zipSize = result.zipSize;
        spinner.succeed(`Build zipped: ${chalk.dim(zipFilePath)} ${chalk.cyan(`(${formatSize(zipSize)})`)}`);
      }
      ...
      this.log(chalk.dim('  File:    ') + chalk.white(`${payload.fileName} (${formatSize(payload.file.length)})`));
      this.log();
