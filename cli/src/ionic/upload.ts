...
export class UploadClient {
          ...
          res.on('end', () => {
            try {
              ...
              if (statusCode >= 400) {
                ...
                reject(new ApiError(this.extractErrorMessage(errorBody, res.statusMessage || 'Unknown error'), statusCode, errorBody));
                return;
              }
  ...
  private validatePayload(payload: UploadPayload): void {
    ...
    if (!payload.nativeVersion) throw new ValidationError('Native version is required', 'nativeVersion');
    if (!payload.file || payload.file.length === 0) throw new ValidationError('Build file is required', 'file');
  }
