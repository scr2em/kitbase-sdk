...
export async function buildAndZip(options: BuildOptions): Promise<BuildAndZipResult> {
	const cwd = process.cwd();
	const outputDir = options.skipBuild
		? findWebOutputDir(options.outputDir)
		: await buildIonicWeb(options);
...
export function validateZipFile(filePath: string): void {
	...
	if (stats.size > maxSize) {
		throw new ValidationError(
			`Zip file too large (${Math.round(stats.size / 1024 / 1024)}MB). Max is 100MB.`,
		);
	}
