...
import { useBooleanFlag, useStringFlag, useNumberFlag, useJsonFlag } from "./use-flag";
...
describe("useBooleanFlag", () => {
	...
	it("should return default value while loading", () => {
		const { result } = renderHook(() => useBooleanFlag("test-flag"), {
	...
	it("should fetch flag value on mount", async () => {
		...
		expect(mockClient.getBooleanValue).toHaveBeenCalledWith("test-flag", undefined);
	});

	it("should pass context to the client", async () => {
		const context = { targetingKey: "user-123", plan: "premium" };
		const { result } = renderHook(() => useBooleanFlag("test-flag", { context }), {
			wrapper: createWrapper(mockClient),
		});
		...
		expect(mockClient.getBooleanValue).toHaveBeenCalledWith("test-flag", context);
	});
	...
	it("should refetch when flagKey changes", async () => {
		const { result, rerender } = renderHook(({ flagKey }) => useBooleanFlag(flagKey), {
			wrapper: createWrapper(mockClient),
		...
		await waitFor(() => {
			expect(mockClient.getBooleanValue).toHaveBeenCalledWith("flag-2", undefined);
		});
	});
	...
	it("should refetch when context changes (default behavior)", async () => {
		...
		await waitFor(() => {
			expect(mockClient.getBooleanValue).toHaveBeenCalledWith("test-flag", {
				targetingKey: "user-2",
			});
		});
	});
	...
	it("should provide refetch function", async () => {
		...
		const initialCallCount = (mockClient.getBooleanValue as ReturnType<typeof vi.fn>).mock.calls
			.length;
		...
		expect(
			(mockClient.getBooleanValue as ReturnType<typeof vi.fn>).mock.calls.length,
	...
	it("should unsubscribe from events on unmount", async () => {
		...
		const { unmount } = renderHook(() => useBooleanFlag("test-flag"), {
			wrapper: createWrapper(mockClient),
		});
	...
	it("should handle context equal when both are undefined", async () => {
		...
		expect((mockClient.getBooleanValue as ReturnType<typeof vi.fn>).mock.calls.length).toBe(
			callCount,
		);
	});
	...
	it("should handle context equal when one is undefined", async () => {
		const { result, rerender } = renderHook(
			({ context }) => useBooleanFlag("test-flag", { context }),
			{
				wrapper: createWrapper(mockClient),
				initialProps: {
					context: { targetingKey: "user-1" } as { targetingKey: string } | undefined,
				},
			},
		);
...
describe("useStringFlag", () => {
	...
	it("should fetch string flag value", async () => {
		const { result } = renderHook(() => useStringFlag("string-flag"), {
			wrapper: createWrapper(mockClient),
		});
		...
		expect(mockClient.getStringValue).toHaveBeenCalledWith("string-flag", undefined);
	});

	it("should pass context to the client", async () => {
		const context = { targetingKey: "user-123" };
		const { result } = renderHook(() => useStringFlag("string-flag", { context }), {
			wrapper: createWrapper(mockClient),
		});
		...
		expect(mockClient.getStringValue).toHaveBeenCalledWith("string-flag", context);
	});
});
...
describe("useNumberFlag", () => {
	...
	it("should fetch number flag value", async () => {
		const { result } = renderHook(() => useNumberFlag("number-flag"), {
			wrapper: createWrapper(mockClient),
		});
		...
		expect(mockClient.getNumberValue).toHaveBeenCalledWith("number-flag", undefined);
	});

	it("should pass context to the client", async () => {
		const context = { targetingKey: "user-123" };
		const { result } = renderHook(() => useNumberFlag("number-flag", { context }), {
			wrapper: createWrapper(mockClient),
		});
		...
		expect(mockClient.getNumberValue).toHaveBeenCalledWith("number-flag", context);
	});
});
...
describe("useJsonFlag", () => {
	...
	it("should fetch json flag value", async () => {
		...
		const { result } = renderHook(() => useJsonFlag("json-flag"), {
			wrapper: createWrapper(mockClient),
		});
		...
		expect(mockClient.getJsonValue).toHaveBeenCalledWith("json-flag", undefined);
	});

	it("should pass context to the client", async () => {
		const context = { targetingKey: "user-123" };
		const defaultValue = { default: true };
		const { result } = renderHook(() => useJsonFlag("json-flag", { context }), {
			wrapper: createWrapper(mockClient),
		});
		...
		expect(mockClient.getJsonValue).toHaveBeenCalledWith("json-flag", context);
	});
});
...
describe("Hook Re-render on Flag Change", () => {
	it("should update the returned value when flag changes", async () => {
		...
		const { result } = renderHook(() => useBooleanFlag("dark-mode"), {
			wrapper: createWrapper(mockClient),
		});
	...
	it("should update string flag value when flag changes", async () => {
		...
		const { result } = renderHook(() => useStringFlag("experiment"), {
			wrapper: createWrapper(mockClient),
		});
	...
	it("should update number flag value when flag changes", async () => {
		...
		const { result } = renderHook(() => useNumberFlag("max-items"), {
			wrapper: createWrapper(mockClient),
		});
	...
	it("should also update when ready event fires", async () => {
		...
		const { result } = renderHook(() => useBooleanFlag("feature-flag"), {
			wrapper: createWrapper(mockClient),
		});
