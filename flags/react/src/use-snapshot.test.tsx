...
describe("useFlagSnapshot", () => {
	...
	it("should return loading state initially", () => {
		const { result } = renderHook(() => useFlagSnapshot(), { wrapper: createWrapper(mockClient) });
	...
	it("should pass context to the client", async () => {
		...
		const { result } = renderHook(() => useFlagSnapshot({ context }), {
	...
	it("should refetch when context changes (default behavior)", async () => {
		const { result, rerender } = renderHook(({ context }) => useFlagSnapshot({ context }), {
			wrapper: createWrapper(mockClient),
	...
	it("should provide refetch function", async () => {
		...
		expect((mockClient.getSnapshot as ReturnType<typeof vi.fn>).mock.calls.length).toBeGreaterThan(
			initialCallCount,
		);
	});
	...
	it("should refetch on configurationChanged event", async () => {
		...
		await waitFor(() => {
			expect(
				(mockClient.getSnapshot as ReturnType<typeof vi.fn>).mock.calls.length,
	...
	it("should unsubscribe from events on unmount", async () => {
		...
		const { unmount } = renderHook(() => useFlagSnapshot(), { wrapper: createWrapper(mockClient) });
