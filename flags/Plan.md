...
2. the core package, should have 2 modes
   a. Remote evaluation (default): Each flag evaluation makes an API call 1. we take cacheTTL,
   b. Local evaluation: Fetches config once and evaluates flags locally
   i. we let them specifiy the polling internal, default 60s 2. once the backend sends a new configuration onConfigurationChange emits flag and value
