...
import type { FlagConfiguration, FlagDefinition, SegmentDefinition } from "./config-types.js";
...
describe("FlagEvaluator", () => {
	...
	describe("segment operators", () => {
		...
		it("neq operator", () => {
			...
			expect(evaluator.evaluate("test-flag", { plan: "free" }).enabled).toBe(true);
			expect(evaluator.evaluate("test-flag", { plan: "enterprise" }).enabled).toBe(false);
		});
		...
		it("contains operator", () => {
			...
			expect(evaluator.evaluate("test-flag", { email: "user@company.com" }).enabled).toBe(true);
			expect(evaluator.evaluate("test-flag", { email: "user@other.com" }).enabled).toBe(false);
		});
		...
		it("not_contains operator", () => {
			...
			expect(evaluator.evaluate("test-flag", { email: "user@company.com" }).enabled).toBe(true);
			expect(evaluator.evaluate("test-flag", { email: "user@competitor.com" }).enabled).toBe(false);
		});
		...
		it("starts_with operator", () => {
			...
			expect(evaluator.evaluate("test-flag", { userId: "admin_123" }).enabled).toBe(true);
			expect(evaluator.evaluate("test-flag", { userId: "user_123" }).enabled).toBe(false);
		});
		...
		it("ends_with operator", () => {
			...
			expect(evaluator.evaluate("test-flag", { email: "student@university.edu" }).enabled).toBe(
				true,
			);
			expect(evaluator.evaluate("test-flag", { email: "user@company.com" }).enabled).toBe(false);
		});
		...
		it("exists operator", () => {
			...
			expect(evaluator.evaluate("test-flag", { betaOptIn: true }).enabled).toBe(true);
			expect(evaluator.evaluate("test-flag", { betaOptIn: false }).enabled).toBe(true);
		...
		it("not_exists operator", () => {
			...
			expect(evaluator.evaluate("test-flag", { blockedReason: "spam" }).enabled).toBe(false);
		});
		...
		it("in operator", () => {
			...
			expect(evaluator.evaluate("test-flag", { country: "US" }).enabled).toBe(true);
			expect(evaluator.evaluate("test-flag", { country: "CA" }).enabled).toBe(true);
			expect(evaluator.evaluate("test-flag", { country: "DE" }).enabled).toBe(false);
		});
		...
		it("not_in operator", () => {
			...
			expect(evaluator.evaluate("test-flag", { country: "US" }).enabled).toBe(true);
			expect(evaluator.evaluate("test-flag", { country: "CN" }).enabled).toBe(false);
		});
	});
	...
	describe("evaluateAll", () => {
		it("should evaluate all flags", () => {
			...
			expect(results.map((r) => r.flagKey).sort()).toEqual(["flag-a", "flag-b", "flag-c"]);
		});
	});
