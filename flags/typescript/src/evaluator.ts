...
import type { EvaluationContext, EvaluatedFlag, ResolutionReason } from "./types.js";
...
function compareValues(
	...
	const contextStr =
		contextValue !== undefined && contextValue !== null ? String(contextValue) : "";
 ...
 */
function matchesSegment(segment: SegmentDefinition, context: EvaluationContext): boolean {
 ...
 */
function getContextValue(context: EvaluationContext, field: string): unknown {
...
export function evaluateFlag(
	...
	const sortedRules = [...(flagDef.rules || [])].sort((a, b) => a.priority - b.priority);
	...
	for (const rule of sortedRules) {
		if (matchesRule(rule, context, segmentsMap)) {
			const reason: ResolutionReason = rule.segmentKey
				? "TARGETING_MATCH"
				: rule.rolloutPercentage !== null && rule.rolloutPercentage !== undefined
					? "SPLIT"
					: "STATIC";
...
export class FlagEvaluator {
	 ...
	 */
	evaluate(flagKey: string, context?: EvaluationContext): EvaluatedFlag {
