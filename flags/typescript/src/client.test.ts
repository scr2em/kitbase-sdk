...
describe("FlagsClient", () => {
	...
	describe("constructor", () => {
		it("should throw ValidationError when sdkKey is missing", () => {
			...
			expect(() => new FlagsClient({ sdkKey: "" })).toThrow("SDK key is required");
		});
	...
	describe("Remote Evaluation Mode", () => {
		...
		describe("evaluateFlag", () => {
			it("should throw ValidationError when flagKey is missing", async () => {
				...
				await expect(client.evaluateFlag("")).rejects.toThrow("Flag key is required");
			});
		...
		describe("getBooleanValue", () => {
			...
			it("should throw TypeMismatchError for wrong type", async () => {
				...
				const client = new FlagsClient({ sdkKey: "test-token" });
				await expect(client.getBooleanValue("api-url")).rejects.toThrow(TypeMismatchError);
			});
		});
	...
	describe("Local Evaluation Mode", () => {
		...
		describe("segment targeting (local)", () => {
			it("should match segment rules", async () => {
				...
				const premiumResult = await client.getBooleanValue("premium-feature", {
					targetingKey: "user-1",
					plan: "premium",
				});
				...
				const freeResult = await client.getBooleanValue("premium-feature", {
					targetingKey: "user-2",
					plan: "free",
				});
	...
	describe("Error Handling Edge Cases", () => {
		it("should throw FlagNotFoundError on 404 with flagKey in response", async () => {
			...
			const client = new FlagsClient({ sdkKey: "test-token" });
			await expect(client.evaluateFlag("unknown-flag")).rejects.toThrow(FlagNotFoundError);
		});
	...
	describe("Global Default Values", () => {
		...
		describe("when flag is not found", () => {
			it("should use global default when method default is undefined", async () => {
				...
				const result = await client.getStringValue("unknown-flag");
