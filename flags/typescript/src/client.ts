...
export class FlagsClient {
	...
	constructor(config: FlagsConfig) {
		...
		this.enablePersistentCache =
			config.remoteEvaluationCache?.persistent ?? this.isLocalStorageAvailable();
	 ...
	 * @throws {TimeoutError} When the request times out
	 */
	async evaluateFlag(flagKey: string, options?: EvaluateFlagOptions): Promise<EvaluatedFlag> {
		if (!flagKey) {
	...
	private async doEvaluateFlag(
		...
		const payload = this.buildEvaluateFlagPayload(flagKey, options?.context, options?.defaultValue);
		const flag = await this.request<EvaluatedFlag>("/sdk/v1/feature-flags/evaluate", payload);
	 ...
	 */
	async getBooleanValue(flagKey: string, context?: EvaluationContext): Promise<boolean> {
	 ...
	 */
	async getStringValue(flagKey: string, context?: EvaluationContext): Promise<string> {
	 ...
	 */
	async getNumberValue(flagKey: string, context?: EvaluationContext): Promise<number> {
	 ...
	 */
	private buildSnapshotPayload(context?: EvaluationContext): SnapshotPayload | undefined {
		if (!context) {
			return undefined;
		}
	 ...
	 */
	private getEvaluateFlagCacheKey(flagKey: string, context?: EvaluationContext): string {
		const contextKey = context?.targetingKey ?? "anonymous";
